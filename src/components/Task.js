  ...
  (element , index) => {
      return (
          ...
          <div className="listItem">
            <div className="left" onClick={()=>{setStrikeThrough(!strikeThrough); console.log(strikeThrough)}}>{`${index+1} | ${element.title}`}</div>

          <div className="right"><input type="checkbox" id="done" value={index}/><img className="edit" src = {edit} onClick={() => {forEdit(index)}}/><img className="delete" src = {del} onClick={() => {dele(index)}}/>
          </div>
          </div>
          {strikeThrough === true ? <Card currData = {data[index]} /> : <div></div>} 
          </div>
      )
  }
)
